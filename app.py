...
from PIL import Image, ImageTk      # load images, place images
import shelve


path_to_highscore = path.join("/Users/Shared", "YourHighScore")
print(path_to_highscore)
d = shelve.open(path_to_highscore)
try:
...
class Snake(tk.Canvas):
    ...
    def create_objects(self):
        self.create_text(
            70, 12, text=f"Score: {self.score} (Speed: {moves_per_second})", tag="score", fill="#fff", font=10
        )
    ...
    def end_game(self):
        ...
        global highscore, path_to_highscore
        if self.score > highscore:
            highscore = self.score
            d = shelve.open(path_to_highscore)
            d['highscore'] = highscore  # now it is saved on disk.
            d.close()
...
try:
    moves_per_second = 15
    root = tk.Tk()
    root.title(f"SnakeGame (Highscore: {highscore})")
